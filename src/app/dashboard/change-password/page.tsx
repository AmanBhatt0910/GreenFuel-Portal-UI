  ...
  return (
    <div className="mt-2">
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="text-xs font-medium" style={{ color: strength > 1 ? (strength > 2 ? "rgb(34 197 94)" : "rgb(234 179 8)") : "rgb(239 68 68)" }}>
          {label.text}
        </span>
      </div>
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
  ...
  return (
    <motion.div
      className="min-h-screen flex items-center justify-center bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 px-4 py-12"
      initial="initial"
      animate="animate"
      exit="exit"
    ...
    >
      ...
      >
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <motion.div variants={itemVariants}>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full pl-4 pr-10 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700/50 dark:border-gray-600 dark:text-white transition-all duration-200 ${
                      errors.oldPassword ? "border-red-500 bg-red-50 dark:bg-red-900/10" : "border-gray-300"
                    }`}
                ...
                {errors.oldPassword && (
                  <p className="mt-1.5 text-sm text-red-500 flex items-center">
                    <AlertCircle className="h-3.5 w-3.5 mr-1" /> {errors.oldPassword}
                  </p>
                )}
              </motion.div>
              ...
              <motion.div variants={itemVariants}>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full pl-4 pr-10 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700/50 dark:border-gray-600 dark:text-white transition-all duration-200 ${
                      errors.newPassword ? "border-red-500 bg-red-50 dark:bg-red-900/10" : "border-gray-300"
                    }`}
                ...
                {errors.newPassword && (
                  <p className="mt-1.5 text-sm text-red-500 flex items-center">
                    <AlertCircle className="h-3.5 w-3.5 mr-1" /> {errors.newPassword}
                  </p>
                )}
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-1.5">
                    {requirements.map((req, index) => (
                      <div key={index} className="flex items-center">
                        <div className={`rounded-full h-3.5 w-3.5 flex items-center justify-center ${req.met ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`}>
                          {req.met && <div className="h-2 w-2 rounded-full bg-white"></div>}
                        </div>
                        <span className={`ml-2 text-xs ${req.met ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
                          {req.text}
                        </span>
                      </div>
                    ))}
              ...
              <motion.div variants={itemVariants}>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full pl-4 pr-10 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700/50 dark:border-gray-600 dark:text-white transition-all duration-200 ${
                      errors.confirmPassword ? "border-red-500 bg-red-50 dark:bg-red-900/10" : "border-gray-300"
                    }`}
                ...
                {errors.confirmPassword && (
                  <p className="mt-1.5 text-sm text-red-500 flex items-center">
                    <AlertCircle className="h-3.5 w-3.5 mr-1" /> {errors.confirmPassword}
                  </p>
                )}
                ...
                {confirmPassword && (
                  <div className="mt-2 flex items-center">
                    <div className={`h-2 w-2 rounded-full ${confirmPassword === newPassword ? 'bg-green-500' : 'bg-red-500'}`}></div>
                    <span className={`ml-1.5 text-xs ${confirmPassword === newPassword ? 'text-green-600 dark:text-green-400' : 'text-red-500'}`}>
              ...
              <motion.div variants={itemVariants}>
                ...
                >
                  <span className={`absolute inset-0 w-0 bg-white/20 transition-all duration-500 ease-out group-hover:w-full ${isLoading ? 'w-full' : ''}`}></span>
        ...
        <motion.p 
          ...
          variants={itemVariants}
        >
          Need help? <Link href="/support" className="text-blue-600 hover:underline dark:text-blue-400">Contact Support</Link>
        </motion.p>
      </motion.div>
    </motion.div>
  );
