...
const mockChatMessages: ChatMessage[] = [
  {
    ...
    sender: 2, // requester
    message: "Hi, I wanted to ask about the status of my request. When can I expect approval?",
  ...
  },
  {
    ...
    message: "Hello! I'm reviewing your request now. Could you provide more details about why you need this equipment?",
  ...
  },
  {
    ...
    chatroom: "chat-room-1",
    sender: 2, // requester
    message: "Sure, I need this for the new project we're starting next month. Our current equipment doesn't support the software requirements for the project.",
 ...
 */
export default function useApprovalDetails({ id }: UseApprovalDetailsProps): UseApprovalDetailsReturn {
  ...
  const fetchBusinessUnitDetails = async (buId: number): Promise<BusinessUnit> => {
    ...
    try {
      ...
      console.error(`Error fetching business unit details for ID ${buId}:`, err);
      return { 
        id: buId,
  ...
  const fetchDesignationDetails = async (designationId: number): Promise<Designation> => {
    ...
    try {
      ...
      console.error(`Error fetching designation details for ID ${designationId}:`, err);
      return { 
        id: designationId,
  ...
  const fetchApprovalDetails = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch approval details'));
  ...
  const handleApprove = async () => {
    try {
      ...
      // await api.put(`/approval-requests/${id}/`, {
      //   status: "Approved",
      //   rejection_reason: null
      // });
  ...
  const handleStartChat = async () => {
    ...
    try {
      ...
      const welcomeMessage: ChatMessage = {
        ...
        message: `Chat started by ${enrichedForm?.user_name ? 'approver with ' + enrichedForm.user_name : 'approver'}. You can now discuss this request.`,
        timestamp: new Date().toISOString(),
        read: false
      };
