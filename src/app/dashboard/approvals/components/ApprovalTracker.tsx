            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {form.department_name || form.department} • {form.approval_category}
                </p>
              </div>
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-muted-foreground">
                  Level {form.current_level} of {form.max_level}
                </span>
              </div>
              ...
              <div className="relative">
                ...
                <div 
                  ...
                  style={{ 
                    width: `${form.status?.toLowerCase() === 'approved' 
                      ? '100' 
                      : form.status?.toLowerCase() === 'rejected'
                        ? ((form.current_level - 1) / form.max_level * 100)
                        : (form.current_level / form.max_level * 100)}%` 
                  }}
                ></div>
                
                {/* Level indicators */}
                <div className="flex justify-between items-center w-full absolute top-4">
                  {Array.from({ length: form.max_level }, (_, index) => {
                    const levelNumber = index + 1;
                    const isCompleted = form.current_level > levelNumber;
                    const isCurrent = form.current_level === levelNumber;
                    const isRejected = form.status?.toLowerCase() === 'rejected' && isCurrent;
                    ...
                    return (
                      <div key={index} className="flex flex-col items-center mt-1">
                        ...
                        >
                          ...
                          {isRejected && <XCircle className="h-2 w-2 text-red-500" />}
                        </div>
                        <span 
              ...
              <div className="mt-12 pt-1 flex items-center space-x-2 text-sm">
                <span className="text-muted-foreground">Approval Path:</span>
                <span className="font-medium">Requester</span>
                <ArrowRight className="h-3 w-3 text-muted-foreground" />
                <span className={form.current_level >= 1 ? "font-medium" : "text-muted-foreground"}>Team Lead</span>
                <ArrowRight className="h-3 w-3 text-muted-foreground" />
                <span className={form.current_level >= 2 ? "font-medium" : "text-muted-foreground"}>Manager</span>
                {form.max_level > 2 && (
                  <>
                    <ArrowRight className="h-3 w-3 text-muted-foreground" />
                    <span className={form.current_level >= 3 ? "font-medium" : "text-muted-foreground"}>Director</span>
                  </>
                )}
                {form.max_level > 3 && (
                  <>
                    <ArrowRight className="h-3 w-3 text-muted-foreground" />
                    <span className={form.current_level >= 4 ? "font-medium" : "text-muted-foreground"}>CFO</span>
                  </>
                )}
              </div>
              ...
              <div className="mt-4 rounded-md bg-gray-50 dark:bg-gray-800/50 p-3 text-sm">
                {form.status?.toLowerCase() === 'approved' ? (
                  ...
                  <p className="text-red-600 dark:text-red-400 flex items-center">
                    ...
                    This request was rejected at level {form.current_level}.
                    {form.rejection_reason && (
                      <span className="ml-1">Reason: {form.rejection_reason}</span>
                    )}
                  </p>
                ) : (
                  <p className="text-blue-600 dark:text-blue-400 flex items-center">
                    <Clock className="h-4 w-4 mr-2" />
                    This request is currently awaiting approval at level {form.current_level} ({form.current_level === 1 ? 'Team Lead' : form.current_level === 2 ? 'Manager' : form.current_level === 3 ? 'Director' : 'CFO'}).
                  </p>
                )}
              </div>
            </div>
